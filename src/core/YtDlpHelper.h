...
public:
     ...
     * on extraction. On desktop this is the DASH video+audio pair (same tracks
     * as playback, so a seek keeps the chosen resolution); on embedded a single
     * H.264 stream. The child's stdout goes to a temp file; call
     * read_direct_urls() later to collect the result. Returns the temp file
     ...
     * URL is present, *out_audio_url (the audio stream). Returns true when the
     * file contains at least one URL. */
    static bool read_direct_urls(const std::string& file, std::string* out_video_url,
                                 std::string* out_audio_url);

    /* Launches "ffmpeg -ss <start_ms> -i <video_url> [-ss <start_ms> -i
     * <audio_url>] -c copy -f matroska" and returns the read end of its stdout
     * pipe. With both URLs the DASH video and audio are merged at the offset
     * (full resolution); with only a video URL a single progressive file is
     * range-seeked. Used to restart playback at an arbitrary position of a
     * video that only yt-dlp can extract (YouTube). Returns fd >= 0 on success,
     * or -1 on failure (*out_pid is valid then). */
    int launch_seek_stream(const std::string& video_url, const std::string& audio_url,
